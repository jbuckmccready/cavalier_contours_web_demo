...
export function plineArrayToJsonStr(
  array: Float64Array,
  isClosed: boolean
): string {
  const obj = {
...
export function jsonStrToPlineArray(
  jsonStr: string
): { array: Float64Array; isClosed: boolean } {
  try {
...
export function createPlinePropertiesSetRustCodeStr(
  plines: Polyline[]
): string {
  return (
    "[" +
    plines
      .map((pline) => createPlineTestPropertiesRustCodeStr(pline))
      .join(", ") +
    "]"
  );
}
