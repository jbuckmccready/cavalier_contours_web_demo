...
impl Polyline {
    ...
    pub fn translate(&mut self, x_offset: f64, y_offset: f64) {
        self.0.translate(x_offset, y_offset);
    }
    ...
    pub fn arcs_to_approx_lines_data(&self, error_distance: f64) -> Box<[f64]> {
        ...
        let mut data = Vec::with_capacity(2 * pline_result.len());
        for v in pline_result.iter() {
            data.push(v.x);
            data.push(v.y);
        }
...
fn pline_vertexes_to_data(pline: &cavc::Polyline<f64>) -> Vec<f64> {
    let mut data = Vec::with_capacity(3 * pline.len());
    for v in pline.iter() {
        data.push(v.x);
        data.push(v.y);
        data.push(v.bulge);
